...
class ApplicationController < ActionController::Base
  ...
  def cal_total_asset_value
    ...
    # 计算貸款項目的本金与利息
    @loan_value_sum = sum_money_of('loan_items')+get_total_loan_interest_ntd_array.sum.to_i
    # 加总计算净值
...
=end
  ...
  # 获取燕大星苑·红树湾的房产总价
  def get_hongshuwan_price
    AssetItem.find(93).amount.to_i
  end
...
=end
  ...
  def  get_price_from_api(url='https://api.huobi.br.com/market/history/kline?period=1min&size=1&symbol=btchusd',param_name='btc_price',symbol=nil)
    ...
    if root and root["code"] == "200" and body and body["data"] and body["data"][0] and body["data"][0]["close"]
      return body["data"][0]["close"]
    else
      return value_of(param_name).to_f
    end
  end
  ...
  def get_kline_data(period="15min",size=100,symbol="btcusdt")
    ...
    root = ActiveSupport::JSON.decode(resp.to_json)
    body = ActiveSupport::JSON.decode(root["body"])
    if root and root["code"] == "200" and body and body["data"]
  ...
  # 更新火币USDT/HUSD交易对汇率
  def update_husd_rate(usd2twd=value_of("exchange_rates_USD").to_f)
    husd = get_husd_price
    Param.find_by_name("exchange_rates_HUSD").update_attribute(:value,format("%.4f", usd2twd/husd))
    Param.find_by_name("ex_rates_USDT_to_HUSD").update_attribute(:value,husd)
  end

  def usd2twd
    do_exchange_from_api("USD","TWD")
  end

  def usd2cny
    do_exchange_from_api("USD","CNY")
  end

  def cny2twd
    do_exchange_from_api("CNY","TWD")
  end

  def krw2twd
    do_exchange_from_api("KRW","TWD")
  end

  def do_exchange_from_api(fromCode, toCode, money=1)
    url = "https://ali-waihui.showapi.com/waihui-transform?fromCode=#{fromCode}&toCode=#{toCode}&money=#{money}"
    authorization = "APPCODE de9f4a29c5eb4c73b0be619872e18857"
    resp = get_ssl_response(url,authorization)
    root = ActiveSupport::JSON.decode(resp.to_json)
    if root["message"] == "OK" # 不管什么情况，如果发生异常，则返回0
      body = ActiveSupport::JSON.decode(root["body"])
      return body["showapi_res_body"]["money"].to_f
    else
      return 0
    end
  end
  ...
  # 更新比特币总投资预算
  def update_btc_total_budget
    total_budget = 0
    [5,134,119,11].each do |index|
      total_budget += AssetItem.find(index).to_ntd
    end
    Param.find(206).update_attribute(:value, total_budget)
  end
  ...
  def get_exchange_rate_from_web( param_name = 'exchange_rates_MCY' )
    ...
    download_data = get_remote_files(host_name,"/#{url}",'get_exchange_rate_from_web.txt',80,false)
  ...
  def update_all_members_classification_by_histories_count_month
    ...
    Member.find(:all, :conditions => [ "area_id = ?", session[:default_area_id] ]).each do |m|
      ...
      if ( m.career != '6' and m.career != '7' and m.career != '99' )  # 不是 學生學員 職工學員 群眾
          ...
          if m.is_new_member and m.histories_count_month >= 1
          ...
          elsif ( m.get_age > 18 and m.histories_count_3month >= 6 ) or ( m.get_age <= 18 and m.histories_count_3month >= 6 )
          ...
          elsif ( m.get_age > 18 and m.histories_count_3month >= 2 ) or ( m.get_age <= 18 and m.histories_count_3month >= 2 )
          ...
          elsif ( m.get_age > 18 and m.histories_count_6month >= 1 ) or ( m.get_age <= 18 and m.histories_count_6month >= 1 )
  ...
  def get_remote_files( host_name, url, save_file_path, port=80, exe_save=false )
    begin
...
class Fixnum
    ...
    def class_type_arr
        [ ['训读会','read'],['主日礼拜','sunday_service'],['二世礼拜','sunday_service_2g'],['室内活动','indoor'],['户外活动','outdoor'],['培训活动','training'],['一般讲课','normal'],['半日讲课','half'],['一日灵修','1day'],['两日灵修','2day'],['三日灵修','3day'],['四日灵修','4day'],['七日灵修','7day'],['十日灵修','10day'],['個別陪談','link'],['個別解惑','answer'],['養育課程','parenting'],['电话联系','tel'],['短信分享','wenxin'],['小组团契','association'],['小组会议','meeting'],['闲聊叙旧','keep_friendship']]
    end
    ...
    asset_item_records.each do |r|
      case r.currency
        ...
        when 'KRW'
          exchange_rate = value_of('exchange_rates_KRW').to_f
        when 'HUSD'
          exchange_rate = value_of('exchange_rates_HUSD').to_f
      end
      result += r.amount*exchange_rate
    end
    return result / value_of('exchange_rates_'+currency).to_f
  ...
  # 計算貸款利息总值
  def get_total_loan_interest_ntd_array( to_time = Date.today )
    total_loan_interests = []
    AssetItem.all(:include => :asset, :conditions => ["assets.code = ? and asset_belongs_to_id = ?", 'loan_items', 1], :order => id).each do |item|
      #計算每日利息
      day_interest = item.ntd_amount * (item.loan_interest_rate/100.0) / 365
      #計算已经经过的天数
      pass_days = (to_time.to_date - item.loan_begin_date.to_date).to_i
      #計算利息
      total_loan_interests << day_interest * pass_days
    end
  ...
  def prepare_my_asset_var
    ...
    # 数字资产总值
    @total_digital_currency = sum_money_of('digital_currency', 'NTD', 1)
    @total_flow_assets += @total_digital_currency
    # 貸款項目总值
    ...
    @net_asset_value = @total_asset_value #@total_current_assets + @total_fixed_assets
    # 每月最多能存多少新台币
  ...
  def sim_blessing_arr
    [ ['2005祝福式',100],['2009祝福式',140],['2010祝福式',150],['2011祝福式',160],['2012祝福式',170],['2013祝福式',180],['2014祝福式',190],['2015祝福式',200],['2016祝福式',210] ]
  end
