...
class HuobiPro
  ...
  def initialize(access_key,secret_key,account_id,server_name,signature_version="2")
      @access_key = access_key
      @secret_key = secret_key
      @signature_version = signature_version
      @account_id = account_id
      @server_name = server_name
      @uri = URI.parse "https://#{@server_name}/"
      @header = {
  ...
  ## 获取 Market Depth 数据
  # {"ts"=>1564380752832, "status"=>"error", "err-code"=>"bad-request", "err-msg"=>"invalid topic market.btchusd.depth"}
  ...
  ## 查询某个订单详情
  def order_status(order_id) # order_status(order_id,market)
  ...
  def open_orders(symbol,side)
    params ={
      "symbol" => symbol,
      "types" => "#{side}-limit", # 订单类型，包括buy-market, sell-market, buy-limit, sell-limit, buy-ioc, sell-ioc, buy-limit-maker, sell-limit-maker
  ...
  def util(path,params,request_method)
    ...
    url = "https://#{@server_name}#{path}?#{Rack::Utils.build_query(h)}"
